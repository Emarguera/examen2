...
function MdGesture($$MdGestureHandler, $$rAF, $timeout) {
  ...
  function checkDistanceAndEmit(name) {
    return function(ev, pointer) {
      if (pointer.distance < this.state.options.maxDistance) {
        this.dispatchEvent(ev, name, pointer);
      }
    };
  }
    ...
    .handler('swipe', {
      ...
      onEnd: function (ev, pointer) {
        ...
        if (Math.abs(pointer.velocityX) > this.state.options.minVelocity &&
          ...
          id = pointer.directionX == 'left' ? '$md.swipeleft' : '$md.swiperight';
          this.dispatchEvent(ev, id);
        }
          ...
          id = pointer.directionY == 'up' ? '$md.swipeup' : '$md.swipedown';
          this.dispatchEvent(ev, id);
        }
      }
    });
...
function MdGestureHandler() {
   ...
   * @param id the name of the custom event to send (eg 'click' or '$md.drag')
   * @param eventPointer the pointer object that matches this event.
   */
  function jQueryDispatchEvent(srcEvent, id, eventPointer) {
    eventPointer = eventPointer || pointer;
    var eventObj = new angular.element.Event(id);
   ...
   * @param eventPointer the pointer object that matches this event.
   */
  function nativeDispatchEvent(srcEvent, id, eventPointer) {
    eventPointer = eventPointer || pointer;
    var eventObj;

    if (id === 'click' || id == 'mouseup' || id == 'mousedown' ) {
      eventObj = document.createEvent('MouseEvents');
      eventObj.initMouseEvent(
        id, true, true, window, srcEvent.detail,
      ...
      eventObj.initCustomEvent(id, true, true, {});
    }
    eventObj.$material = true;
    eventObj.pointer = eventPointer;
    eventObj.srcEvent = srcEvent;
